...
//
// Safety properties:
//   - Only writes inside the active workspace (via lib/workspace resolveInside).
//   - Backs up the previous file to .mybeam/history/<ts>/<path> before overwriting.
//   - Uses atomic write (write to temp, then rename).
//   - Refuses ambiguous find-and-replace (must match exactly once).
//   - Appends a log line to .mybeam/memory/CHANGES.md.
...
function applyOne(edit) {
  ...
  if (edit.kind === 'new') {
    if (exists) {
      // Overwrite is allowed; back it up first.
  ...
  if (edit.kind === 'edit') {
    ...
    // Count occurrences of `find` in the file.
    let count = 0, from = 0;
    while (true) {
